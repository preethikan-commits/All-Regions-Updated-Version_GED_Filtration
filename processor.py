...
def _read_excel_robust(uploaded_file, **kwargs):
    """
    pd.read_excel wrapper that falls back to the 'calamine' engine if the
    default 'openpyxl' engine fails to parse the file. Some Shopee Seller
    Center exports embed a worksheet view/pane XML attribute (e.g. an invalid
    'activePane' value) that openpyxl refuses to parse but calamine handles
    fine.
    """
    uploaded_file.seek(0)
    try:
        ...
        uploaded_file.seek(0)
        return pd.read_excel(uploaded_file, engine="calamine", **kwargs)


def _excel_sheet_names_robust(uploaded_file):
    """pd.ExcelFile(...).sheet_names with the same calamine fallback."""
    uploaded_file.seek(0)
    try:
        return pd.ExcelFile(uploaded_file).sheet_names
    except Exception:
        uploaded_file.seek(0)
        return pd.ExcelFile(uploaded_file, engine="calamine").sheet_names
...
def load_file(uploaded_file, sheet_name=0, header=0, dtype=None):
    ...
    if name.endswith(".csv"):
        ...
        return _read_excel_robust(uploaded_file, sheet_name=sheet_name, header=header, dtype=dtype)


def _detect_sc_report_header_row(raw_df, max_scan=10):
    """
    Scan the first max_scan rows of a raw (header=None) DataFrame to find the
    row containing the actual SC (Seller Center) Report column headers —
    identified by common SKU / Product ID marker columns. Returns the 0-based
    row index, or 2 as a fallback (Shopee SC Report exports place the real
    headers on row 3 — rows 1-2 hold template/meta info, not data).
    """
    markers = {
        "sellersku", "seller sku", "sku", "product id", "parent sku",
    ...
    for i in range(min(max_scan, len(raw_df))):
        ...
        if any(m in row_vals for m in markers):
            return i
    return 2  # documented default: headers on row 3 (0-indexed: 2)


def load_sc_report(uploaded_file, sheet_name=0, dtype=None):
    """
    Load the Shopee Seller Center (SC) Report, auto-detecting the true header
    row by scanning for SellerSKU / SKU / Product ID marker columns instead of
    always assuming row 1. Falls back to row 3 (index 2) if no marker row is
    found, since Shopee SC Report exports place the real column headers on
    row 3 (rows 1-2 hold internal template/meta info, not real data).
    """
    name = uploaded_file.name.lower()
    uploaded_file.seek(0)
    if name.endswith(".csv"):
        raw = pd.read_csv(uploaded_file, header=None, dtype=str)
        header_row = _detect_sc_report_header_row(raw)
        uploaded_file.seek(0)
        return pd.read_csv(uploaded_file, header=header_row, dtype=dtype)
    else:
        raw = _read_excel_robust(uploaded_file, sheet_name=sheet_name, header=None)
        header_row = _detect_sc_report_header_row(raw)
        uploaded_file.seek(0)
        return _read_excel_robust(
            uploaded_file, sheet_name=sheet_name, header=header_row, dtype=dtype
        )
...
def _load_sheet_raw(uploaded_file, sheet_name):
    ...
    uploaded_file.seek(0)
    return _read_excel_robust(uploaded_file, sheet_name=sheet_name, header=None)
...
def load_ecom_tracker(uploaded_file, region):
    ...
    try:
        sheet_names = _excel_sheet_names_robust(uploaded_file)
    except Exception as e:
...
def load_am_exclusion(uploaded_file, region):
    ...
    try:
        df = _read_excel_robust(uploaded_file, sheet_name=sheet_name)
    except Exception:
        uploaded_file.seek(0)
        df = _read_excel_robust(uploaded_file, sheet_name=0)
    return df
...
def _prepare_common_data(
    ...
    # SC Report exports from Shopee Seller Center place the real column
    # headers on row 3 (rows 1-2 are internal template/meta info), so we use
    # the header-detecting loader rather than assuming row 1 is the header.
    sc_df = load_consolidated(sc_report_file, load_sc_report, dtype=str)
...
def process_shopee_voucher_eligibility(
    ...
    Step 6/7 - Voucher eligibility: a Product ID flagged in ANY of the four
             mandatory columns is always ineligible. Otherwise, a voucher with
             no Inclusion Keyword configured is eligible for every remaining
             PID; a voucher with one or more Inclusion Keywords (including
             "Open for all", which is just another Exclusion Remark value)
             is eligible only if the PID is flagged in the matching dynamic
             "<keyword>_PID" column — every other dynamic PID remark column
             is ignored for that voucher's decision.
    """
    common = _prepare_common_data(
    ...
    for vc in voucher_configs:
        ...
        def _evaluate(row, _kws=keywords):
            ...
            # Case 1 — no Inclusion Keyword configured at all for this
            # voucher: no keyword restriction was set up, so every PID that
            # cleared the mandatory checks is eligible.
            if not _kws:
                return "Yes-Eligible"

            # Case 2 — one or more Inclusion Keyword(s) selected. This
            # includes "Open for all" (it is just another Exclusion Remark
            # value from the Ecom Tracker, selected the same way as any other
            # keyword such as "MAX 25% VC"): the PID is eligible only if it
            # is flagged in the matching dynamic "<keyword>_PID" column for
            # at least one of the selected keywords. Every other dynamic PID
            # remark column is ignored and left out of the decision entirely
            # — a voucher restricted to "Open for all" is evaluated only
            # against the "Open for all_PID" column and nothing else.
            for kw in _kws:
                col_name = f"{kw}_PID"
